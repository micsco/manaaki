...
function focusKeepMealButton(button: HTMLButtonElement | null) {
  button?.focus()
}
...
export function MealPlanDialog({
  ...
  async function save() {
    if (saving || removing || confirmingRemoval) return
  ...
  async function remove() {
    if (!entry || saving || removing) return
    setRemoving(true)
    setError(null)
    try {
      const result = await deleteOneApiHouseholdsMealplansItemIdDelete({
        path: { item_id: entry.id },
      })
      if (result.error) throw new Error("Removal failed")
      await qc.invalidateQueries({ queryKey: ["mealplan"] })
      toastManager.add({ title: "Removed from meal plan", description: entryTitle(entry) })
      onClose()
    } catch {
      setError("Couldn't remove this meal. Please try again.")
    } finally {
      setRemoving(false)
    }
  }
  ...
  return (
    <Dialog.Root
      open
      onOpenChange={open => {
        if (!open && !saving && !removing) onClose()
      }}
    >
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 z-50 mobile-dialog max-h-[90dvh] w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-gray-800 bg-gray-900 p-6 text-gray-100 shadow-xl">
          ...
          <Dialog.Description className="mt-2 text-sm text-gray-400">
            {confirmingRemoval && entry
              ? `${entryTitle(entry)} · ${new Date(`${entry.date}T00:00:00`).toLocaleDateString("en-GB", { weekday: "long", day: "numeric", month: "long" })} · ${entry.entryType ?? "dinner"}`
              : (recipe?.name ??
          ...
          {confirmingRemoval ? (
            <div className="mt-5 space-y-4">
              <p className="text-sm text-gray-400">
                This removes the meal from your plan. The recipe will still be available.
              </p>
              <div className="flex flex-wrap justify-end gap-3">
                <button
                  type="button"
                  ref={focusKeepMealButton}
                  disabled={removing}
                  onClick={() => {
                    setConfirmingRemoval(false)
                    setError(null)
                  }}
                  className="min-h-11 rounded-full px-4 py-2 text-gray-300 disabled:opacity-50"
                >
                  Keep meal
                </button>
                <button
                  ...
                  disabled={removing}
                  onClick={() => void remove()}
                  className="min-h-11 rounded-full bg-red-600 px-5 py-2 font-medium text-white hover:bg-red-500 disabled:opacity-50"
                >
              ...
              {entry && (
                <div className="border-t border-gray-800 pt-3">
                  <button
                    ...
                    className="min-h-11 rounded-full px-4 py-2 text-sm text-red-400 hover:bg-gray-800 disabled:opacity-50"
                  >
                    Remove from plan
                  </button>
                </div>
