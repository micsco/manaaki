...
export function BuildShoppingListDialog({
  ...
  return (
    <Dialog.Root
      open={open}
      onOpenChange={next => {
        if (!next && !building) onClose()
      }}
    >
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 z-40 bg-black/60" />
        <Dialog.Popup className="fixed inset-x-0 bottom-0 z-50 mx-auto mobile-sheet w-full max-w-lg rounded-t-2xl bg-gray-900 p-5 sm:top-1/2 sm:bottom-auto sm:-translate-y-1/2 sm:rounded-2xl">
